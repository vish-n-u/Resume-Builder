			...
			<div className="p-8">
				...
				{data.professional_summary && (
					<section className="mb-8">
						...
						<p className="text-base text-gray-700 ">{data.professional_summary}</p>
					</section>
				)}
								...
								<div key={index} className="experience-item relative pl-6 border-l border-gray-200">
									...
									<div className="flex justify-between items-start mb-2">
										<div>
											...
											<p className="text-base font-medium" style={{ color: accentColor }}>{exp.company}</p>
										</div>
										<div className="text-base text-gray-500 bg-gray-100 px-3 py-1 rounded">
											{formatDate(exp.start_date)} - {exp.is_current ? "Present" : formatDate(exp.end_date)}
										</div>
									</div>
									{exp.description && (
										<div className="text-base text-gray-700 leading-relaxed mt-3 whitespace-pre-line">
											{exp.description}
										</div>
									)}
								</div>
						...
						<div className="space-y-6">
							{data.project.map((p, index) => (
								<div key={index} className="project-item relative pl-6 border-l border-gray-200" style={{borderLeftColor: accentColor}}>
									...
									{p.description && (
										<div className="text-base text-gray-700 leading-relaxed mt-3">
											{p.description}
										</div>
									)}
								</div>
							...
							<div className="space-y-4">
								{data.education.map((edu, index) => (
									<div key={index} className="education-item">
										...
										<p className="text-base" style={{ color: accentColor }}>{edu.institution}</p>
						...
						<section>
							...
							<div className="flex flex-wrap gap-2">
								{data.skills.map((skill, index) => (
									<span
										key={index}
										className="px-3 py-1 text-base text-white rounded-full"
